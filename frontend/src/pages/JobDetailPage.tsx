...
export default function JobDetailPage() {
  ...
  const handleRunAgain = async () => {
    if (!job?.strategyId) return
    setError("")
    setRunning(true)
    try {
      const newJob = await runBacktest(job.strategyId)
      navigate(`/dashboard/jobs/${newJob.jobId}`)
    } catch (err) {
      setError(extractMessage(err, "Failed to start new run"))
      setRunning(false)
    }
  }
      ...
      <div className="max-w-2xl mx-auto py-12">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            disabled={!job.strategyId || running}
            className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {running ? "Starting..." : "Try again"}
          </button>
        </div>
      </div>
    )
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <StrategySummary definition={job.definition} />
      </div>
